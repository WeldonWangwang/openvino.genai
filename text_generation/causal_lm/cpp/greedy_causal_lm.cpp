...
void infer_task(ov::InferRequest lm, int64_t out_token, int64_t SPECIAL_EOS_TOKEN, size_t seq_len, TextStreamer text_streamer, ov::Tensor position_ids, float* logits, size_t vocab_size, bool enable_stream, int max_sequence_length)
{
    constexpr size_t BATCH_SIZE = 1;
    ...
    std::cout << "\nRest Tokens take time: " << diff.count() << " s\n";
    std::cout << "Rest Token/s: " << diff.count() / seq_len << std::endl;
    if (enable_stream) {
...
int main(int argc, char* argv[]) try {
    if (argc != 6) {
        throw std::runtime_error(std::string{"Usage: "} + argv[0] + " <MODEL_DIR> '<PROMPT>' <DEVICE_NAME> <NUM_DEVICES> <MAX_OUTPUT_TOKEN>");
    }
    ...
    for (int i = 0; i < num_thread; i++) {
        ...
        std::cout << "input token size: " << seq_len << std::endl;
        ...
        auto end = std::chrono::system_clock::now();
        std::chrono::duration<double> diff = end-start;
        std::cout << "First token time: " << diff.count() << " s\n";
        ...
        threads.emplace_back(std::thread(infer_task, lm, out_token, SPECIAL_EOS_TOKEN, seq_len, text_streamer, position_ids, logits, vocab_size, enable_stream, max_output_token_nums));
        enable_stream = false;
    }
