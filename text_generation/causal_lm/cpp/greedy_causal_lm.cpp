...
void infer_task(char* prompt, ov::CompiledModel tokenizer_compile_model, ov::CompiledModel detokenizer_compile_model, ov::CompiledModel compile_model, std::shared_ptr<ov::Model> tokenizer_model,  bool enable_stream, int max_sequence_length, int num_iters)
{
    constexpr size_t BATCH_SIZE = 1;
        ...
        for (int i = 0; i < num_iters+1; i++) {
            ...
            std::cout << log_tag << " input token size: " << seq_len << std::endl;
            ...
            std::cout << log_tag << " seq_len = " << seq_len << std::endl;
            end = std::chrono::system_clock::now();
            diff = end-start;
            std::cout << log_tag << " Rest Tokens take time: " << diff.count() << " s\n";
            std::cout << log_tag << " Rest Token/s: " << diff.count() / seq_len << std::endl;
            if (enable_stream) {
...
int main(int argc, char* argv[]) try {
    if (argc != 7) {
        throw std::runtime_error(std::string{"Usage: "} + argv[0] + " <MODEL_DIR> '<PROMPT>' <DEVICE_NAME> <NUM_DEVICES> <MAX_OUTPUT_TOKEN> <NUM ITERS>");
    }
    ...
    ov::CompiledModel detokenizer_compile_model = core.compile_model(std::string{argv[1]} + "/openvino_detokenizer.xml", "CPU");
    ov::AnyMap config = {}; 
    ...
    for (int i = 0; i < num_thread; i++) {
        threads.emplace_back(std::thread(infer_task, argv[2], tokenizer_compile_model, detokenizer_compile_model, compile_model, tokenizer_model, enable_stream, max_output_token_nums, std::stoi(argv[6])));
        enable_stream = false;
    }
